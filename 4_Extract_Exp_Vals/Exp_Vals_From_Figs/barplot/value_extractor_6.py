...
with st.sidebar:
    ...
    edited_calib = st.data_editor(
        ...
        key="df_editor_calibration",
    )
    st.session_state.df_calibration, active_idx = sync_single_selection(edited_calib.copy(), "selected_calib_idx_memory")
...
if st.session_state.image_cv is not None:
    ...
    with col_table:
        ...
        st.caption("체크박스 선택은 1개 행만 유지되도록 고정했습니다. 표 스크롤은 Streamlit rerun 구조상 완전 고정은 어렵지만, 표 높이를 고정해 덜 흔들리게 했습니다.")
        # 💡 "Group" 컬럼 추가
        ...
        st.session_state.df_extracted, selected_row_idx = sync_single_selection(
            edited_df.copy(), "selected_row_idx_memory"
        )
        ...
        if img_rgb is not None:
            ...
            with img_col_left:
                ...
                canvas_result = st_canvas(
                    ...
                    update_streamlit=False, # 💡 [해결 1] 드래그 중 실시간 렌더링을 끄고 마우스를 뗄 때만 데이터 전송 (렉 감소)
                    ...
                    if len(objs) > 0:
                        ...
                        if last_obj["type"] == "rect":
                            ...
                            if dr_w > 10 and dr_h > 10:
                                ...
                                # 💡 [해결 2] 방금 지정한 영역과 똑같으면 무시! (무한 RUNNING 루프 차단)
            ...
            with img_col_right:
                if st.session_state.get("zoom_region") is None:
                    ...
                    try:
                        # 💡 [핵심 해결] 브라우저 CSS(use_column_width)에 의존하지 않고, 
                        # 파이썬에서 물리적으로 왼쪽 화면(canvas_display_width)과 똑같은 크기로 이미지를 변환합니다.
                        ratio = z_w / canvas_display_width
                        display_h = int(z_h / ratio)
                        
                        # PIL을 이용해 물리적 리사이징 수행
                        resized_crop_pil = Image.fromarray(cropped_img).resize((canvas_display_width, display_h))
                        
                        # 💡 use_column_width=True 제거
                        ...
                        if clicked is not None:
                            # 💡 물리적으로 크기를 맞췄으므로 ratio만 곱해주면 100% 완벽한 원본 좌표로 복원됩니다.
                            click_crop_x = int(clicked["x"] * ratio)
                            click_crop_y = int(clicked["y"] * ratio)
                            
                            # 돋보기 안에서의 좌표를 진짜 원본 좌표로 복원
            ...
            if selected_row_idx is not None:
                if st.button("수동 조작한 커서 위치로 값 적용", use_container_width=True, key="btn_cursor_apply"):
...
# python -m streamlit run /Users/kogeon/python_projects_path/LNPDB_Articles_Extraction/4_Extract_Exp_Vals/Exp_Vals_From_Figs/value_extractor_5.py