...
def render_single_select_table(
    ...
    auto_scroll_to_selected: bool = False,
):
    """
    ...
    if use_aggrid:
        ...
        if auto_scroll_to_selected:
            scroll_idx = remembered_idx if remembered_idx is not None and 0 <= remembered_idx < len(grid_df) else None
            if scroll_idx is not None:
                grid_options["ensureDomOrder"] = True
                grid_options["onFirstDataRendered"] = {
                    "function": f"params.api.ensureIndexVisible({int(scroll_idx)}, 'middle'); params.api.getDisplayedRowAtIndex({int(scroll_idx)})?.setSelected(true);"
                }
    ...
    if auto_scroll_to_selected:
        remembered_idx = st.session_state.get(memory_key)
        if remembered_idx is not None and 0 <= remembered_idx < len(work_df):
            st.caption(f"선택 행 자동 이동은 AgGrid에서 가장 안정적입니다. 현재 선택 행: {remembered_idx + 1}행")
...
def build_display_image(selected_row_idx=None):
    ...
    pending_map = {}
    for item in st.session_state.get("pending_clicks", []):
        row_idx = int(item.get("row_idx", -1))
        if row_idx >= 0:
            pending_map[row_idx] = {
                "x_pixel": item.get("x_pixel"),
                "y_pixel": item.get("y_pixel"),
            }

    for idx, row in st.session_state.df_extracted.iterrows():
        pending_item = pending_map.get(idx)

        if pending_item is not None and pd.notna(pending_item.get("x_pixel")) and pd.notna(pending_item.get("y_pixel")):
            px_x, px_y = int(pending_item["x_pixel"]), int(pending_item["y_pixel"])
            is_pending = True
        elif pd.notna(row.get("x_pixel")) and pd.notna(row.get("y_pixel")):
        ...
        if idx == selected_row_idx:
            color, radius, alpha = (0, 255, 255), 4, 0.40
        else:
            color, radius, alpha = (0, 255, 0), 2, 0.22

        overlay = display_img.copy()
        cv2.circle(overlay, (px_x, px_y), radius, color, -1)

        if is_pending:
            cv2.circle(overlay, (px_x, px_y), radius + 3, color, 1)
...
# =========================
# 오른쪽 돋보기/클릭 패널 분리 렌더링 함수
# =========================
#@st.fragment
def render_zoom_right_panel(img_rgb, selected_row_idx, is_horizontal, canvas_display_width, orig_w, orig_h):
...
if st.session_state.image_cv is not None:
    ...
    with col_img:
        ...
        if img_rgb is not None:
            ...
            # ==========================================
            with img_col_right:
                render_zoom_right_panel(img_rgb, selected_row_idx, is_horizontal, canvas_display_width, orig_w, orig_h)
...
# python -m streamlit run /Users/kogeon/python_projects_path/LNPDB_Articles_Extraction/4_Extract_Exp_Vals/Exp_Vals_From_Figs/value_extractor_9.py